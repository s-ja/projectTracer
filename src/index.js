...
program
  ...
  .option('-f, --format <format>', '출력 형식 (json, csv, markdown)', DEFAULT_OUTPUT_FORMAT)
...
if (!options.owner || !options.repo || !options.user) {
  console.error('오류: 저장소 소유자(-o), 저장소 이름(-r), 사용자명(-u)은 필수입니다.');
  program.help();
  process.exit(1);
}
...
if (!options.issues && !options.prs && !options.commits && !options.stats && !options.portfolio) {
  options.all = true;
  options.issues = true;
  options.prs = true;
  options.commits = true;
...
async function main() {
  ...
  try {
    ...
    if (options.issues) {
      ...
      const issues = await api.fetchUserIssues(options.owner, options.repo, options.user);
    ...
    if (options.prs) {
      ...
      const prs = await api.fetchUserPRs(options.owner, options.repo, options.user);
    ...
    if (options.commits) {
      ...
      const commits = await api.fetchUserCommits(options.owner, options.repo, options.user);
